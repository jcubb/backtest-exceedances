...
Each metric is one row of the ``metrics`` table below: the ``varlib`` method
name, the VaR percentile, and (for the exponentially weighted methods) a
half-life in years. Result columns encode all of that:

    var_<method>_<years>y_<percentile>[_hl<half_life>y]
...
def _column_name(method: str, years: int, percentile: float, half_life_years: float | None) -> str:
    """Build a result column name encoding method, window, threshold, half-life."""
    column = f"var_{method}_{years}y_{round(percentile * 100, 6):g}"
...
def main() -> None:
    ...
    years = 3
    # (varlib method, VaR percentile, half-life in years). The percentile is 0.99
    # for plain VaR and 0.9745 for the expected-shortfall methods (their 97.45%
    # shortfall equals the 99% VaR under normality). The half-life is None for the
    # equal-weight methods. Add rows here to compare, e.g. EW methods at different
    # half-lives.
    metrics = [
        ("SD-Scaled", 0.99, None),
        ("Historical", 0.99, None),
        ("ES-Equiv-Historical", 0.9745, None),
        ("EW-SD-Scaled", 0.99, 1.0),
        ("EW-Historical", 0.99, 1.0),
        ("EW-ES-Equiv-Historical", 0.9745, 1.0),
    ]

    results = returns.to_frame()
    var_columns = []
    for method, percentile, half_life in metrics:
        column = _column_name(method, years, percentile, half_life)
