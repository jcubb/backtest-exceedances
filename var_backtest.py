...
Reads the simulated returns produced by generate_data.py, then adds five
rolling VaR estimates, each built with a different methodology:

    (1) var_param_3y  - trailing 3yr std dev, scaled by the 99% normal z-score
    (2) var_hist_3y   - empirical worst 1% return over a trailing 3yr window
    (3) var_es_3y     - mean of the worst 2.55% of returns over 3yr
                        (97.45% expected shortfall == 99% VaR under normality)
    (4) var_ewma_5y   - one-year half-life std dev over a trailing 5yr window,
                        scaled by the 99% normal z-score
    (5) var_brw_5y    - Boudoukh-Richardson-Whitelaw weighted-quantile: the
                        worst-1% empirical return over 5yr, where observations
                        are weighted with a one-year half-life
...
# Tail probability whose conditional mean (expected shortfall) equals the 99%
# normal VaR: norm.pdf(z99) / 0.01 corresponds to the 97.45% ES level.
ES_TAIL_PROBABILITY = 0.0255

INPUT_FILE = Path(__file__).parent / "returns.xlsx"
OUTPUT_FILE = Path(__file__).parent / "var_backtest_results.csv"

Z_SCORE_99 = norm.ppf(CONFIDENCE)  # ~2.3263
...
def rolling_var(
    ...
    """Compute a rolling 99% VaR series for one of the supported methods.
    ...
    half_life_years : float
        Half-life for the weighted methods ("ewma" and "brw"), in years.
    """
    ...
    for day in range(window_days, len(values)):
        ...
        var_estimates[day] = _window_var(window, method, half_life_days)

    return pd.Series(var_estimates, index=returns.index, name=f"var_{method}_{years}y")


def _window_var(window: np.ndarray, method: str, half_life_days: float) -> float:
    """Compute a single VaR figure (positive loss) for one trailing window."""
    if method == "parametric":
        return Z_SCORE_99 * window.std(ddof=1)

    if method == "historical":
        return -np.quantile(window, 1 - CONFIDENCE)

    if method == "expected_shortfall":
        threshold = np.quantile(window, ES_TAIL_PROBABILITY)
...
def main() -> None:
    ...
    results["var_param_3y"] = rolling_var(returns, "parametric", years=3)
    results["var_hist_3y"] = rolling_var(returns, "historical", years=3)
